 ...
 * dependency is there. Since this is a proper dependency, a deregistration will
 * automatically deactivate any components that depend on this service. Once
 * something is mapped to a service it leverages the DS runtime to handle the
 * highly complex ordering issues between different components.
 *
 ...
 *
 * The {@link AggregateState} also registers the cardinality that it detects for
 * each state. The cardinality is the number of values that were registered for
 * a state. It will prefix the state id with a hash ({@code #}) and register the
 * total number of values that it found in other services. The {@code %} prefix
 * registers the total number of unique values.
 *
...
public interface AggregateState {
	...
	/**
	 * The name of the service property key that identifies a service providing
	 * aggregate states. The mapped value cardinality can be multiple.
	 */
	 ...
	 * </pre>
	 */
	String CAPABILITY_NAME = "osgi.aggregate.state";
